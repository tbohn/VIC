    ...
    while (!feof(paramfile)) {
        if (cmdstr[0] != '#' && cmdstr[0] != '\n' && cmdstr[0] != '\0') {
            ...
            *************************************/
            ...
            else if (strcasecmp("ALBEDO_H2O_SURF", optstr) == 0) {
                sscanf(cmdstr, "%*s %lf", &param.ALBEDO_H2O_SURF);
            }
...
validate_parameters()
{
    ...
    if (!(param.ALBEDO_H2O_SURF >= 0 && param.ALBEDO_H2O_SURF <= 1)) {
        log_err("ALBEDO_H2O_SURF must be defined on the interval [0,1] (-)")
    }
    // Surface Emissivities
