        ...
        veg_lib_struct      *veg_lib)
{
    ...
    ******************************************/
    ...
    if (options.IRRIGATION) {
        for (iveg = 0; iveg <= Nveg; iveg++){
            if (veg_con[iveg].irr_active && veg_con[iveg].Cv > 0.0) {
                Cv = veg_con[iveg].Cv;
                Nbands = options.SNOW_BAND;
                for (band = 0; band < Nbands; band++) {
                    if (soil_con->AreaFract[band] > 0) {
                        if (options.CROPSPLIT &&
                            ...
                            all_vars->cell_subtiles[iveg][0][band].irr_apply = false;
                            cell = &(all_vars->cell_subtiles[iveg][1][band]);
                            veg_var = &(all_vars->veg_var_subtiles[iveg][1][band]);
                            snow = &(all_vars->snow_subtiles[iveg][1][band]);
                        }
                        else {
                ...
                for (band = 0; band < Nbands; band++) {
                    ...
                    if (soil_con->AreaFract[band] > 0) {
                        ...
                        if (options.CROPSPLIT && veg_con[iveg].crop_split) {
                            ...
                            energy = &(all_vars->energy_subtiles[iveg][cridx][band]);
                        }
                        else {
                            cell = &(all_vars->cell[iveg][band]);
                            veg_var = &(all_vars->veg_var[iveg][band]);
                        ...
                        ******************************************/
                        ...
                        /* Irrigation applied */
                        cell->irr_applied = cell->irr_demand;
                        ...
                        if (options.IRRIGATION &&
                            ...
                            if (irr_demand_total > (force->prec[NR] +
                                                    force->irr_run[NR] +
                                                    force->irr_with[NR])) {
                                cell->irr_applied *= (force->prec[NR] +
                                                      force->irr_run[NR] +
                                                      force->irr_with[NR]) /
                                                     irr_demand_total;
                            }
    ...
    ****************************/
    ...
    if (options.LAKES && lake_con->lake_idx >= 0) {
        ...
        lake_var->influx = lake_con.maxvolume - lake_var.volume - (lake_var->runoff_in + lake_var->baseflow_in)
