                     ...
                     double             swq)
{
    ...
    if (swq < 0.001 && force->air_temp[NR] > 7) {
        ...
        else if (veg_lib->ithresh == IRR_WP)
            irr_sm_thresh = soil_con->Wpwp[thresh_idx]; // wilting point
        else
        ...
        else if (veg_lib->itarget == IRR_WP)
            irr_sm_target = soil_con->Wpwp[thresh_idx];
        else
            irr_sm_target = soil_con->Wcr[thresh_idx];
